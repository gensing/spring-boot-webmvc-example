...
public class PostServiceImpl implements PostService {
    ...
    public PostDto.PostResponse get(long postId) {
        final PostEntity postEntity = postRepository.findById(postId).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND_DATA));
    ...
    public long insert(PostDto.PostRequest postRequest, SecurityDto.UserInfo sessionInfo) {
        final PostEntity postEntity = postMapper.toPost(postRequest, sessionInfo.getId());
    ...
    public void update(long postId, PostDto.PostPutRequest postPutRequest, SecurityDto.UserInfo sessionInfo) {
        ...
        final PostEntity savedPostEntity = postRepository.findById(postId)
                .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND_DATA));

        if (!sessionInfo.checkId(savedPostEntity.getMember().getId())) {
    ...
    public void delete(long postId, SecurityDto.UserInfo sessionInfo) {
        ...
        final PostEntity saved = postRepository.findById(postId)
                .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND_DATA));
